...
describe('Cliente Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Dependencia query and add missing value', () => {
      const cliente: ICliente = { id: 456 };
      const dependencia: IDependencia = { id: 58970 };
      cliente.dependencia = dependencia;

      const dependenciaCollection: IDependencia[] = [{ id: 33533 }];
      jest.spyOn(dependenciaService, 'query').mockReturnValue(of(new HttpResponse({ body: dependenciaCollection })));
      const additionalDependencias = [dependencia];
      const expectedCollection: IDependencia[] = [...additionalDependencias, ...dependenciaCollection];
      jest.spyOn(dependenciaService, 'addDependenciaToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ cliente });
      comp.ngOnInit();

      expect(dependenciaService.query).toHaveBeenCalled();
      expect(dependenciaService.addDependenciaToCollectionIfMissing).toHaveBeenCalledWith(dependenciaCollection, ...additionalDependencias);
      expect(comp.dependenciasSharedCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const cliente: ICliente = { id: 456 };
      const dependencia: IDependencia = { id: 66082 };
      cliente.dependencia = dependencia;
      ...
      expect(comp.dependenciasSharedCollection).toContain(dependencia);
    });
  });
  ...
  describe('Tracking relationships identifiers', () => {
    describe('trackDependenciaById', () => {
      it('Should return tracked Dependencia primary key', () => {
        const entity = { id: 123 };
        const trackResult = comp.trackDependenciaById(0, entity);
