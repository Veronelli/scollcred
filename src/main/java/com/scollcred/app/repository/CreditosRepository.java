...
public interface CreditosRepository extends JpaRepository<Creditos, Long> {
    @Query(value="SELECT cred.* FROM CREDITOS AS cred LEFT JOIN CLIENTE AS clie  ON clie.ID = cred.ID WHERE clie.NOMBRE = :name", nativeQuery = true)
    public List<Creditos> findByClienteName(@Param("name") int nombre);
}
